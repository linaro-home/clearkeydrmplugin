...
#include "ion/ion_allocator_helper.h"

namespace clearkeydrm {

#define DEFAULT_ION_HEAP	5

using namespace android;

class AesCtrDecryptorSecureTest : public ::testing::Test {
  protected:
    ...
    status_t attemptDecrypt(const Key& key, const Iv& iv, const uint8_t* source,
        ...
        return decryptor.secureDecrypt((const char *)keyVector.array(), iv,
                                       source, destination,
    ...
    void attemptDecryptExpectingSuccess(const Key& key, const Iv& iv,
        ...
        media::IonAllocator ionBuffer;
        native_handle_t *native_hdl;
        native_hdl = native_handle_create(1, 0);
        if (native_hdl == NULL)
            return;

        native_hdl->data[0] = ionBuffer.Allocate(totalSize, DEFAULT_ION_HEAP);

        size_t bytesDecrypted = 0;
        ASSERT_EQ(android::OK, attemptDecrypt(key, iv,
                                              encrypted, (uint8_t*)native_hdl,
                                              subSamples, numSubSamples,
                                              &bytesDecrypted));
        EXPECT_EQ(totalSize, bytesDecrypted);
        EXPECT_EQ(0, memcmp(ionBuffer.Map(), decrypted, totalSize));

        native_handle_close(native_hdl);
        native_handle_delete(native_hdl);
...
TEST_F(AesCtrDecryptorSecureTest, DecryptsContiguousEncryptedBlock) {
...
TEST_F(AesCtrDecryptorSecureTest, DecryptsAlignedBifurcatedEncryptedBlock) {
    const size_t kTotalSize = 64;
...
TEST_F(AesCtrDecryptorSecureTest, DecryptsUnalignedBifurcatedEncryptedBlock) {
    const size_t kTotalSize = 64;
    const size_t kNumSubsamples = 2;
...
TEST_F(AesCtrDecryptorSecureTest, DecryptsOneMixedSubSample) {
...
TEST_F(AesCtrDecryptorSecureTest, DecryptsAlignedMixedSubSamples) {
...
TEST_F(AesCtrDecryptorSecureTest, DecryptsUnalignedMixedSubSamples) {
    const size_t kTotalSize = 80;
    const size_t kNumSubsamples = 2;
...
TEST_F(AesCtrDecryptorSecureTest, DecryptsComplexMixedSubSamples) {
    const size_t kTotalSize = 72;
